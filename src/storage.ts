...
import { extension, makeArchive } from './archive';
...
export async function saveDraft(collection: Collection, draft: Draft): Promise<Collection> {
  ...
  try {
    ...
    await Storage.setItem(KEY, JSON.stringify({ ...updated, entries: updated.entries.map(e => ({ ...e, artwork: relative(e.artwork), reference: relative(e.reference) })) }));
    return updated;
  } catch (error) {
